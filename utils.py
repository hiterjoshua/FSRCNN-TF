...
def preprocess_real3w(HR, LR):
    ...
    # image = Image.open(HR).convert('L')
    # (width, height) = image.size
    # label_ = np.array(list(image.getdata())).astype(np.float).reshape((height, width)) / 255
    # image.close()
    
    # image = Image.open(LR).convert('L')
    # (width, height) = image.size
    # input_ = np.array(list(image.getdata())).astype(np.float).reshape((height, width)) / 255
    # image.close()
    image_HR = cv2.imread(HR)
    HR_yuv = cv2.cvtColor(image_HR, cv2.COLOR_BGR2YUV)
    label_, u, v = cv2.split(HR_yuv)
    label_ = label_.astype(np.float) / 255

    image_LR = cv2.imread(LR)
...
def prepare_data_real3w(config):
    sess = config.sess
    image_size, label_size, stride, scale = config.image_size, config.label_size, config.stride, config.scale

    HR_path = os.path.join(config.dataset, 'HR')
    LR_path = os.path.join(config.dataset, 'LR')

    HR_data = sorted(glob.glob(os.path.join(HR_path, "*.png")))
    LR_data = sorted(glob.glob(os.path.join(LR_path, "*.png")))
    ...
    for i in tqdm(range(len(LR_data))):
    # for i in tqdm(range(500)): # For quick verify add by hukunlei 20210712
        if i % 3 == 0:
            continue

        input_, label_ = preprocess_real3w(HR_data[i], LR_data[i])
        ...
        if len(input_.shape) == 3:
            ...
            input_ = input_.reshape([image_size, image_size, 1])
            label_ = label_.reshape([label_size, label_size, 1])
...
def train_input_setup(config):
    ...
    # Load data path
    if FLAGS.real3w:
        print("Beginning real3w reading...")
        data = prepare_data_real3w(config)
        print("Real3w reading finished...")
        return
    else:
        data = prepare_data(sess, dataset=config.data_dir)
